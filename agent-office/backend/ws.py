"""
ws.py — WebSocket handler: orquestra session_manager e runner para o ciclo
completo de uma mensagem.

Suporta modo coordinator: detecta marcadores [AGENTE ACTIVO: nome] no stream
e emite eventos active_agent separados para o frontend actualizar o office view.
"""
...
async def websocket_endpoint(websocket: WebSocket, db: aiosqlite.Connection) -> None:
    ...
    try:
        while True:
            ...
            # Buffer para detecção do marcador [AGENTE ACTIVO: nome]
            marker_buf = ""
            ...
            async for chunk in runner.invoke(agent, content, cli_session_id):
                ...
                if chunk_type == "session_id":
                    ...
                    if not marker_resolved:
                        ...
                        if m:
                            # Marcador encontrado — emite evento e limpa o marcador do conteúdo
                            ...
                            marker_buf = ""
                        # ainda a bufferisar — não envia nada ainda
                    else:
