...
export function createChat(messagesEl, callbacks = {}) {
  ...
  /**
   * Mostra a mensagem de arranque com 4 sugestões clicáveis.
   *
   * @param {(text: string) => void} onSuggestion - chamado ao clicar numa sugestão
   */
  function showWelcomeMessage(onSuggestion) {
    const SUGGESTIONS = [
      'Iniciar um novo projecto',
      'Continuar um projecto existente',
      'Explorar uma ideia',
    ...
    el.className = 'message assistant';
    el.setAttribute('role', 'article');
    el.setAttribute('data-agent', 'coordinator');
    el.style.setProperty('--agent-color', 'var(--color-coordinator)');

    const meta = document.createElement('div');
    meta.className = 'message-meta';
    meta.innerHTML = `
      <span class="meta-agent" style="color:var(--color-coordinator)">Coordinator</span>
      <span class="meta-time">${formatTime(new Date().toISOString())}</span>
    `;

    const bubble = document.createElement('div');
    bubble.className = 'message-bubble';
    bubble.textContent = 'O que vamos fazer hoje?';
    ...
    SUGGESTIONS.forEach((text) => {
      ...
      btn.addEventListener('click', () => onSuggestion(text));
