...
# Actualiza CLAUDE_WORK_DIR para o directorio actual (portavel entre maquinas)
if (Test-Path $envFile) {
    $envLines = Get-Content $envFile
    $hasWorkDir = $envLines | Where-Object { $_ -match "^CLAUDE_WORK_DIR=" }
    if ($hasWorkDir) {
        $envLines = $envLines -replace "^CLAUDE_WORK_DIR=.*", "CLAUDE_WORK_DIR=$PSScriptRoot"
    } else {
        $envLines += "CLAUDE_WORK_DIR=$PSScriptRoot"
    }
